...
/******/ 	return __webpack_require__(__webpack_require__.s = "../../node_modules/thread-loader/dist/cjs.js?!../../node_modules/babel-loader/lib/index.js?!./src/sandbox/eval/transpilers/svelte/svelte-worker.ts");
/******/ })
/************************************************************************/
/******/ ({
...
/***/ "../../node_modules/thread-loader/dist/cjs.js?!../../node_modules/babel-loader/lib/index.js?!./src/sandbox/eval/transpilers/svelte/svelte-worker.ts":
...
// CONCATENATED MODULE: /Users/uday/Desktop/projects/codesandbox-client/node_modules/thread-loader/dist/cjs.js??ref--9-0!/Users/uday/Desktop/projects/codesandbox-client/node_modules/babel-loader/lib??ref--9-1!./src/sandbox/eval/transpilers/svelte/svelte-worker.ts
const childHandler=new child_handler_ChildHandler('svelte-worker');// Allow svelte to use btoa
// @ts-ignore
self.window=self;function getV3Code(_ref){let code=_ref.code,version=_ref.version,path=_ref.path;self.importScripts("https://unpkg.com/svelte@".concat(version,"/compiler.js"));// @ts-ignore
const _self$svelte$compile=self.svelte.compile(code,{filename:path,dev:true}),js=_self$svelte$compile.js,warnings=_self$svelte$compile.warnings;// Fallback to generate sourcemaps on Svelte 3.13+
// See https://github.com/codesandbox/codesandbox-client/pull/3114
if(!js.map.toUrl){js.map.toUrl=()=>'data:application/json;charset=utf-8;base64,'+btoa(JSON.stringify(js.map));}return{code:js.code,map:js.map,warnings:warnings.map(w=>({fileName:w.fileName,lineNumber:w.start&&w.start.line,columnNumber:w.start&&w.start.column,message:w.message,source:'svelte'}))};}function getV2Code(_ref2){let code=_ref2.code,version=_ref2.version,path=_ref2.path;self.importScripts("https://unpkg.com/svelte@".concat(version,"/compiler/svelte.js"));let error=null;const warnings=[];const _self$svelte$compile2=self.svelte.compile(code,{filename:path,dev:true,cascade:false,store:true,onerror:err=>{error=err;},onwarn:w=>{warnings.push({fileName:w.fileName,lineNumber:w.loc&&w.loc.line,columnNumber:w.loc&&w.loc.column,message:w.message,source:'svelte'});}}),_self$svelte$compile3=_self$svelte$compile2.js,compiledCode=_self$svelte$compile3.code,map=_self$svelte$compile3.map;if(error){throw error;}return{code:compiledCode,map,warnings};}function getV1Code(_ref3){let code=_ref3.code,version=_ref3.version,path=_ref3.path;self.importScripts("https://unpkg.com/svelte@".concat(version,"/compiler/svelte.js"));let error=null;const warnings=[];// @ts-ignore
const _self$svelte$compile4=self.svelte.compile(code,{filename:path,dev:true,cascade:false,store:true,onerror:err=>{error=err;},onwarn:w=>{warnings.push({fileName:w.fileName,lineNumber:w.loc&&w.loc.line,columnNumber:w.loc&&w.loc.column,message:w.message,source:'svelte'});}}),compiledCode=_self$svelte$compile4.code,map=_self$svelte$compile4.map;if(error){throw error;}return{code:compiledCode,map,warnings};}function compile(_x){return _compile.apply(this,arguments);}function _compile(){_compile=asyncToGenerator_default()(function*(data){const code=data.code,path=data.path,version=data.version;let versionCode;if(semver_default.a.satisfies(version,'1.x')){versionCode=getV1Code({code,version,path});}if(semver_default.a.satisfies(version,'2.x')){versionCode=getV2Code({code,version,path});}if(semver_default.a.satisfies(version,'3.x')){versionCode=getV3Code({code,version,path});}if(!versionCode){throw new Error('Unsupported Svelte version');}const _versionCode=versionCode,compiledCode=_versionCode.code,map=_versionCode.map,warnings=_versionCode.warnings;const withInlineSourcemap="".concat(compiledCode,"\n//# sourceMappingURL=").concat(map.toUrl());return{transpiledCode:withInlineSourcemap,warnings};});return _compile.apply(this,arguments);}childHandler.registerFunction('compile',compile);childHandler.emitReady();
...
/******/ });
//# sourceMappingURL=svelte-transpiler.6a4dc218.worker.js.map